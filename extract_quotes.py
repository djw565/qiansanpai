...
def is_good_quote(text: str) -> bool:
    ...
    score = 0
    # 对比/辩证结构
    for pat in [r'不是.{1,10}而是', r'没有.{1,10}只有', r'不是.{1,15}就是']:
        if re.search(pat, text): score += 2
    # 定义性判断
    for pat in [r'(本质|核心|关键).{0,5}(是|在于)', r'(真正|最终).{0,5}(是|在于)', r'(唯一|一切|所有).{1,10}(是|都)']:
        if re.search(pat, text): score += 2
    # 绝对真理
    for pat in [r'从来不', r'永远不', r'任何.{1,5}都', r'才是.{1,20}(的|$)']:
        if re.search(pat, text): score += 1
    # 好结尾
    for pat in [r'而已[。！]?$', r'罢了[。！]?$', r'才是[。！]?$', r'之道[。！]?$']:
        if re.search(pat, text): score += 1
    ...
    # --- 辩证唯物主义关键词（每条 +2）---
    dialectical = [
        '实事求是', '客观规律', '一阶', '二阶', '三阶', '辩证',
        '本质', '自欺欺人', '以身入局', '举一反三', '触类旁通',
        '知行合一', '实践', '元规律', '底层逻辑', '系统', '结构',
        '换位思考', '生态位', '静态', '动态', '场域', '立场',
        '唯物', '矛盾', '对立', '统一', '量变', '质变',
        '信息管理', '产品思维', '一阶思维', '二阶思维',
        '客观', '主观', '闭环', '飞轮', '熵增', '秩序',
        '实事求是地', '知其然', '知其所以然', '看清',
    ...
    # 风格分≥1 且 主题分≥2
    return score >= 1 and relevance >= 2
